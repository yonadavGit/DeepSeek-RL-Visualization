...
% ================= 01 : TITLE =================
\begin{frame}
\eyebrow{Framing \textperiodcentered\ 01}{}
\vspace{8pt}
...
\begin{frame}
\eyebrow{Classical MDP \textperiodcentered\ 03}{background}
...
% ================= 06 : MAPPING TO LLM =================
\begin{frame}
\eyebrow{LLM as MDP \textperiodcentered\ 06}{\S7.1}
...
\begin{frame}
\eyebrow{LLM as MDP \textperiodcentered\ 07}{\S7.1.3}
...
% ================= 08 : LLM AS A TREE =================
\begin{frame}
\eyebrow{LLM as MDP \textperiodcentered\ 08}{}
...
% ================= 09 : BLOCK 1 -- J(THETA) =================
\begin{frame}
\eyebrow{Building Blocks \textperiodcentered\ 09}{\S7.2}
...
\begin{frame}
\eyebrow{Building Blocks \textperiodcentered\ 11}{\S7.2.2}
\frametag{inkdim}{Building block}
...
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 12}{\S7.2.3}
\frametag{neg}{Problem}
...
\begin{frame}
\eyebrow{Building Blocks \textperiodcentered\ 13}{}
\frametag{inkdim}{Building block}
...
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 14}{\S7.3}
...
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 15}{\S7.3}
...
{\small\color{ink}Two versions of the same term get computed --- the true one, and one where the ratio is capped to the trust region. $\min$ always keeps whichever is \textit{smaller}:}
\par\vspace{3pt}
\begin{eqbox}[fontupper=\small,top=4pt,bottom=4pt]
...
\toprule
\textbf{Scenario ($A{=}{+}1$, band $[0.8,1.2]$)} & \textbf{ratio} & \textbf{true} & \textbf{capped} & \textbf{min picks} \\
\midrule
Pushed further, past the margin & 1.5 & 1.5 & 1.2 & \color{accent}capped --- flat \\
Drifted the wrong way & 0.7 & 0.7 & 0.8 & \color{neg}true --- uncapped \\
\bottomrule
\end{tabular}
\par\vspace{3pt}
\begin{callout}
\clabel{accentB}{The one-line version}
{\small\color{ink}Clipping caps the reward for overshooting a good move. It never caps the penalty for a bad one.}
\end{callout}
\end{frame}

% ================= 17 : IS THIS STILL CORRECT =================
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 18}{}
...
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 19}{\S7.3.2}
...
% ================= 18 : PPO -- ASSEMBLED =================
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 20}{\S7.3.1}
\frametag{accentB}{Approach}
\headline{Proximal Policy Optimization (PPO) --- the complete update rule}
\begin{eqbox}[fontupper=\normalsize,top=9pt,bottom=9pt]
$L_{\text{PPO}}(\theta) = \mathbb{E}_t\Big[\min\big(\text{ratio}_t(\theta)\,\tilde A_t,\ \ \text{clip}(\text{ratio}_t(\theta),1{-}\varepsilon,1{+}\varepsilon)\,\tilde A_t\big)\Big]$\\[8pt]
{\small where $\ \tilde A_t = \tilde G_t - V_\phi(s_t), \qquad \tilde r_t = r_t - \beta\,\mathrm{KL}_t(\pi_\theta\Vert\pi_{\text{ref}})$}
\end{eqbox}
\par\vspace{10pt}
...
\begin{frame}
\eyebrow{GRPO \textperiodcentered\ 21}{\S7.4}
\frametag{neg}{Problem}
...
\begin{frame}
\eyebrow{GRPO \textperiodcentered\ 22}{\S7.4}
\frametag{accentB}{Approach}
...
% ================= 19 : GRPO MECHANISM =================
\begin{frame}
\eyebrow{GRPO \textperiodcentered\ 23}{\S7.4.1}
...
\begin{tabular}{@{} l l l @{}}
\toprule
\textbf{Completion} & \textbf{Final answer} & \textbf{Reward} \\
\midrule
\color{accentB}A & 11 & 1 \\
\color{neg}B & 10 & 0 \\
\color{accentB}C & 11 & 1 \\
\color{neg}D & 16 & 0 \\
\bottomrule
\end{tabular}
\par\vspace{10pt}
{\normalsize\color{ink}Group mean $=0.5$. The verifier only ever said 0 or 1 --- it's the comparison against the group that turns that into a signal with both sign and magnitude: A, C reinforced; B, D suppressed.}
\end{frame}

% ================= 20 : GRPO -- ASSEMBLED =================
\begin{frame}
\eyebrow{GRPO \textperiodcentered\ 24}{\S7.4.2 / \S7.4.3}
\headline{GRPO --- the complete update rule}
\begin{eqbox}[fontupper=\normalsize,top=9pt,bottom=9pt]
$L_{\text{GRPO}}(\theta) = \mathbb{E}_i\Big[\min\big(\text{ratio}_i(\theta)A_i,\ \ \text{clip}(\text{ratio}_i(\theta),1{-}\varepsilon,1{+}\varepsilon)A_i\big)\Big] \,-\, \beta\cdot\mathrm{KL}(\pi_\theta\Vert\pi_{\text{ref}})$\\[8pt]
...
% ================= 21 : APPROACH -- RLVR =================
\begin{frame}
\eyebrow{RLVR \textperiodcentered\ 25}{\S7.5}
\frametag{accentB}{Approach}
...
\begin{frame}
\eyebrow{DeepSeek-R1 \textperiodcentered\ 26}{\S7.6}
...
% ================= 22 : RESULT -- R1-ZERO =================
\begin{frame}
\eyebrow{DeepSeek-R1 \textperiodcentered\ 27}{\S7.6}
...
\begin{frame}
\eyebrow{DeepSeek-R1 \textperiodcentered\ 29}{}
\frametag{inkdim}{Building block}
...
\begin{frame}
\eyebrow{DeepSeek-R1 \textperiodcentered\ 30}{\S7.7}
\frametag{accentB}{Approach}
...
\begin{frame}
\eyebrow{DeepSeek-R1 \textperiodcentered\ 31}{\S7.7}
...
% ================= 24 : IMPLEMENTATION =================
\begin{frame}[fragile]
\eyebrow{Implementation \textperiodcentered\ 34}{\S7.8\,--\,7.10}
...
% ================= 25 : SUMMARY =================
\begin{frame}
\eyebrow{Close \textperiodcentered\ 35}{\S7.15}

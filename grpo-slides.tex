...
% ================= 12b : WHY BASELINE SUBTRACTION IS SAFE =================
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 14}{\S7.2.3}
\headline{Subtracting a baseline never biases the gradient}
{\normalsize\color{ink}$\text{baseline}(s)$ can be \textit{any} function of the state, with one rule: it can't depend on the sampled action $a$. That rule alone is enough to guarantee the swap changes nothing about where training is headed.}
\par\vspace{10pt}
\begin{eqbox}[fontupper=\normalsize,top=8pt,bottom=8pt]
$\mathbb{E}_a\big[\nabla_\theta\log\pi_\theta(a|s)\cdot\text{baseline}(s)\big] \;=\; \text{baseline}(s)\cdot\mathbb{E}_a\big[\nabla_\theta\log\pi_\theta(a|s)\big]$\\[6pt]
{\footnotesize $\mathbb{E}_a\big[\nabla_\theta\log\pi_\theta(a|s)\big] = \sum_a \pi_\theta(a|s)\nabla_\theta\log\pi_\theta(a|s) = \nabla_\theta\sum_a\pi_\theta(a|s) = \nabla_\theta 1 = 0$}\\[6pt]
$\Longrightarrow\ \mathbb{E}_a\big[\nabla_\theta\log\pi_\theta(a|s)\cdot\text{baseline}(s)\big] \;=\; \text{baseline}(s)\cdot 0 \;=\; 0$
\end{eqbox}
\par\vspace{8pt}
\begin{callout}[top=6pt,bottom=6pt]
\clabel{accentB}{Why this settles it}
{\small\color{ink}$\text{baseline}(s)$ is constant with respect to $a$, so it pulls straight out of the expectation --- and what's left over is exactly the log-derivative identity ($\pi_\theta\nabla_\theta\log\pi_\theta=\nabla_\theta\pi_\theta$) hiding inside block 2's Monte Carlo estimate. Same conclusion for \textit{any} choice of $\text{baseline}(s)$: PPO's $V_\phi(s)$ and GRPO's group mean are just two different picks.}
\end{callout}
\end{frame}

% ================= 12c : VARIANCE REDUCTION =================
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 15}{\S7.2.3}
\headline{Unbiased isn't the same as useful}
{\small\color{ink}A batch mixing the easy and hard prompts from two slides ago --- two completions each, both typical for their prompt:}
\par\vspace{4pt}
\small
...
\end{tabular}
\par\vspace{5pt}
{\small\color{ink}Raw reward across this batch: mean $0.5$, variance $0.25$. Baseline-adjusted advantage: mean $0$, variance $0.01$ --- a $25\times$ smaller spread in the very term that scales $\nabla_\theta\log\pi_\theta$ for every sample.}
\par\vspace{5pt}
\begin{calloutwarm}[top=6pt,bottom=6pt]
\clabel{accent}{Variance reduction, not bias removal}
{\small\color{ink}Both are unbiased estimators of the same gradient direction (previous slide) --- the difference is noise. Raw $R$ swings with prompt difficulty alone, so a batch mixing easy and hard prompts is dominated by that swing rather than by which completions were actually better. A good $\text{baseline}(s)$ removes exactly that swing.}
\end{calloutwarm}
\end{frame}

% ================= 13 : BLOCK 4 -- KL DIVERGENCE =================
\begin{frame}
\eyebrow{Building Blocks \textperiodcentered\ 16}{}
...
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 20}{\S7.3}
...
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 21}{}
...
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 22}{\S7.3.2}
...
% ================= 18 : PPO -- ASSEMBLED =================
\begin{frame}
\eyebrow{Baselines \& PPO \textperiodcentered\ 23}{\S7.3.1}
...
\begin{frame}
\eyebrow{GRPO \textperiodcentered\ 25}{\S7.4}
\frametag{accentB}{Approach}
...
% ================= 19 : GRPO MECHANISM =================
\begin{frame}
\eyebrow{GRPO \textperiodcentered\ 26}{\S7.4.1}
...
% ================= 20 : GRPO -- ASSEMBLED =================
\begin{frame}
\eyebrow{GRPO \textperiodcentered\ 27}{\S7.4.2 / \S7.4.3}
...
% ================= 21 : APPROACH -- RLVR =================
\begin{frame}
\eyebrow{RLVR \textperiodcentered\ 28}{\S7.5}
\frametag{accentB}{Approach}
...
\begin{frame}
\eyebrow{DeepSeek-R1 \textperiodcentered\ 31}{\S7.6}
\frametag{neg}{Problem}
...
\begin{frame}
\eyebrow{DeepSeek-R1 \textperiodcentered\ 32}{}
...
\begin{frame}
\eyebrow{DeepSeek-R1 \textperiodcentered\ 36}{\S7.7}
...
% ================= 24 : IMPLEMENTATION =================
\begin{frame}[fragile]
\eyebrow{Implementation \textperiodcentered\ 37}{\S7.8\,--\,7.10}
...
% ================= 25 : SUMMARY =================
\begin{frame}
\eyebrow{Close \textperiodcentered\ 38}{\S7.15}

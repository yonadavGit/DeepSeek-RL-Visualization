...
{\small\color{ink}Same log-derivative identity ($\pi_\theta\nabla_\theta\log\pi_\theta=\nabla_\theta\pi_\theta$) buried inside block 2's Monte Carlo estimate. Because $\text{baseline}(s)$ doesn't depend on $a$, it factors straight out of the expectation --- and what's left is exactly zero: $\mathbb{E}_a\big[\nabla_\theta\log\pi_\theta(a|s)\cdot\text{baseline}(s)\big] = \text{baseline}(s)\cdot 0 = 0$, for \textit{any} choice of $\text{baseline}(s)$. PPO's $V_\phi(s)$ and GRPO's group mean are just two different picks.}
\end{callout}
\end{frame}

...
{\footnotesize\color{ink}Two prompts of very different difficulty --- ``What is $12\times 3$?'' (easy) and ``Prove $\sqrt{2}$ is irrational'' (hard). $\text{baseline}(s)$ here is each prompt's \textit{true} typical score --- unknown in practice; PPO and GRPO will later estimate it two different ways.}
\par\vspace{3pt}
\footnotesize
\begin{tabular}{@{} l l l l @{}}
\toprule
\textbf{Prompt} & \textbf{Reward $R$} & \textbf{true baseline$(s)$} & \textbf{$A=R-\text{baseline}(s)$} \\
\midrule

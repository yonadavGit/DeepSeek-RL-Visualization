...
\begin{eqbox}[fontupper=\normalsize,top=7pt,bottom=7pt]
$L_{\text{PPO}}(\theta) = \mathbb{E}_t\Big[\min\big(\text{ratio}_t(\theta)\,\tilde A_t,\ \ \text{clip}(\text{ratio}_t(\theta),1{-}\varepsilon,1{+}\varepsilon)\,\tilde A_t\big)\Big]$\\[5pt]
{\footnotesize $=\ \dfrac{1}{\sum_i T_i}\displaystyle\sum_{i=1}^{N}\sum_{t=1}^{T_i}\min\big(\text{ratio}_{i,t}(\theta)\,\tilde A_{i,t},\ \ \text{clip}(\text{ratio}_{i,t}(\theta),1{-}\varepsilon,1{+}\varepsilon)\,\tilde A_{i,t}\big)$}\\[5pt]
{\footnotesize where $\ \tilde A_t = \tilde G_t - V_\phi(s_t), \qquad \tilde r_t = r_t - \beta\,\mathrm{KL}_t(\pi_\theta\Vert\pi_{\text{ref}})$}
\end{eqbox}
\par\vspace{10pt}
...
\begin{eqbox}[fontupper=\normalsize,top=7pt,bottom=7pt]
$L_{\text{GRPO}}(\theta) = \mathbb{E}_i\Big[\min\big(\text{ratio}_i(\theta)A_i,\ \ \text{clip}(\text{ratio}_i(\theta),1{-}\varepsilon,1{+}\varepsilon)A_i\big)\Big] \,-\, \beta\cdot\mathrm{KL}(\pi_\theta\Vert\pi_{\text{ref}})$\\[5pt]
{\footnotesize $=\ \dfrac{1}{G}\displaystyle\sum_{i=1}^{G}\min\big(\text{ratio}_i(\theta)A_i,\ \ \text{clip}(\text{ratio}_i(\theta),1{-}\varepsilon,1{+}\varepsilon)A_i\big) \,-\, \beta\cdot\mathrm{KL}(\pi_\theta\Vert\pi_{\text{ref}})$}\\[5pt]
{\footnotesize where $\ A_i = \dfrac{r_i-\text{mean}(r_1,\dots,r_G)}{\text{std}(r_1,\dots,r_G)+\varepsilon}$}
\end{eqbox}
\par\vspace{5pt}
